    ...
    def analyse_image(self, image):
        ...
        messages=[
            {'role': 'system', 'content': 'You are a construction safety expert.'},
            {
                'role': 'user',
                'content': [
                    {'type': 'text', 'text': '''Analyze if there is a person present in this picture and check if they are wearing a 
                    1) high vis pants
                    2) hard hat
                    3) high vis vest
                    4) has NO ppe equipments
                    Return the answer in the following format and it should always start with "<output>":
                    <output>
                        <has-hard-hat>
        ...
        for image_path, img_obj in images.items():
            ...
            for d_obj in img_obj['detections']:
                ...
                try:
                    ...
                    json_info = parse_llm_response(img_info)
                    json_info['timestamp'] = img_obj['timestamp']
                    predictions.append(json_info)
                except Exception as _:
                    llm_fails += 1
                time.sleep(5)

        logger.info(f'[VisionAgent] Saving predictions to assets/image_predictions.json')
        with open('assets/image_predictions.json', 'w', encoding='utf-8') as file:
            json.dump({'predictions': predictions, 'LLM Failure': llm_fails}, file)
